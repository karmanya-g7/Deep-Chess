...
bool outOfTime(){
    if(timeLimitMs <= 0) return false; // 0 or negative = no time limit (fixed-depth search)
    auto elapsed = chrono::duration_cast<chrono::milliseconds>(chrono::steady_clock::now() - searchStart).count();
...
// ---------------------------------------------------------------------
// softLimitMs governs whether it's worth STARTING another iteration; the
// existing timeLimitMs is now the hard ceiling outOfTime() checks mid-search
// (see main()). Set by main() before calling this.
long long softLimitMs = 0;
...
string iterativeDeepening(Board& board, vector<uint64_t>& hist, int maxDepth){
    ...
    for(int d = 1; d <= maxDepth; d++){
        ...
        if(d > 1 && lastIterMs > 0 && softLimitMs > 0){
            long long elapsedSoFar = chrono::duration_cast<chrono::milliseconds>(chrono::steady_clock::now() - searchStart).count();
            if(elapsedSoFar + lastIterMs * 2 > softLimitMs) break;
        }
        ...
        long long elapsed = chrono::duration_cast<chrono::milliseconds>(chrono::steady_clock::now() - searchStart).count();
...
void parseGo(const string &command){
    ...
    movetimeArg = -1; depthArg = -1; hasTimeInfo = false; infiniteMode = false;
    while(ss >> token){
        ...
        // "ponder", "searchmoves" etc. are accepted but not specially handled
    }
}
    ...
    while(getline(cin, line)){
        if(line == "uci"){
            ...
            if(depthArg > 0){
                ...
                timeLimitMs = softLimitMs = 24LL * 60 * 60 * 1000; // depth is the limiting factor, not time
            } else if(movetimeArg >= 0){
                timeLimitMs = softLimitMs = max(10, movetimeArg - 30); // exact target, no soft/hard split needed
            } else if(infiniteMode){
                timeLimitMs = softLimitMs = 24LL * 60 * 60 * 1000; // no clock cutoff; only depth/mate will stop us
            } else if(hasTimeInfo){
                int rem = (board.sideToMove() == Color::WHITE) ? wtime : btime;
                int inc = (board.sideToMove() == Color::WHITE) ? winc : binc;
                softLimitMs = moveBudget(rem, inc, movestogo);
                ...
                timeLimitMs = min((long long)rem - 30, softLimitMs * 4);
                if(timeLimitMs < softLimitMs) timeLimitMs = softLimitMs; // pathologically low rem: moveBudget() already handled the safety floor
            } else {
                timeLimitMs = softLimitMs = 3000; // no clock info given at all - safe default
            }

            vector<uint64_t> hist = gameHistory;
            string bestMoveStr = iterativeDeepening(board, hist, maxDepth);
            if(bestMoveStr.empty()){
                Movelist moves;
                movegen::legalmoves(moves, board);
                if(!moves.empty()) bestMoveStr = uci::moveToUci(moves[0]);
            }
            if(bestMoveStr.empty()) cout << "bestmove 0000" << endl;
            else cout << "bestmove " << bestMoveStr << endl;
        } else if(line == "stop" || line.rfind("ponderhit", 0) == 0){
            // engine is fully synchronous, so there's nothing mid-flight to interrupt
        } else if(line == "quit"){
            break;
        }
    }
